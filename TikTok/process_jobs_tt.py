...
from log_in_tt import log_in  # Assuming log_in is correctly implemented and imported
...
def process_job_link(data_id, link, page):
    ...
    try:
        ...
        if page.query_selector('span.AppliedTag[data-test="appliedTag"]'):
            ...
            mark_job_as_applied('TikTok', data_id, 1)
        else:                               
            ...
            if apply_button:
                ...
                if fill_survey(page):
                    ...
                    if received_message and "We have received your resume." in received_message.inner_text():
                        mark_job_as_applied('TikTok', data_id, 1)  # Mark job as applied
...
def main():
    ...
    jobs = get_unprocessed_jobs('TikTok')
    if not jobs:
